...
#              approximate circumference of an ellipse using arrays with
...
def computeYcoordinates (a, b, xValues):
    ...
    yValues= b* sqrt(1.-(xValues**2/a**2))
    return yValues
...
def calcCircumference(a, b, intervals):
    ...
    #a         - length of the semi major axis
    #b         - length of the semi minor axis
    #deltaX    - the change in x values for every value of x in the array
    #intervals - the number of elements in a list or array
    ...
    circumference=0
    deltaX= a/intervals
    circumference= np.sum(sqrt((yValues[:-1]-yValues[1:])**2+deltaX**2))
